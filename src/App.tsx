...
export default function App() {
  ...
  const createEventsForLog = async (token: string, log: LogItem) => {
    ...
    for (const schedule of datesToSchedule) {
        ...
        // Idempotency: Check if this specific event instance was already created
        ...
        try {
            ...
            if (listRes.ok) {
                ...
                if (listData.items && listData.items.length > 0) {
                    continue; // Already exists, skip insertion
                }
            }
        } catch (e) {
        ...
        {currentTab === 'command' && (
            <footer className="bg-zinc-900/80 backdrop-blur-md w-full py-4 border-t border-emerald-500/20 flex flex-col justify-center items-center gap-4 mt-auto">
              <button 
                ...
                disabled={isSyncing}
                className={`text-emerald-400 font-bold underline font-['Inter'] text-sm transition-all ${isSyncing ? 'opacity-50 cursor-wait' : 'cursor-pointer active:opacity-70 hover:text-emerald-300'}`}>
                 {isSyncing ? 'Syncing...' : 'Close Day & Sync'}
              </button>
