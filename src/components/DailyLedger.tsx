...
export default function DailyLedger(props: DailyLedgerProps) {
  ...
  // --- DIGITAL SCRATCHPAD SEPARATED NATIVE TRACKS ---
  ...
  React.useEffect(() => {
    ...
    // Separate Touch Event Handlers for Stylus Input
    ...
    const moveTouch = (e: Event) => {
        if (!drawing) return;
        const touchEvent = e as TouchEvent;
        if (touchEvent.touches.length === 0) return;
        if (touchEvent.cancelable) touchEvent.preventDefault(); // Locks tablet viewport
        const rect = canvas.getBoundingClientRect();
  ...
  return (
    <section className="flex flex-col gap-6 w-full fade-in">
        ...
        {showScratchpad && (
            <div className="fixed inset-0 z-[100] flex i
