...
export default function DailyLedger(props: DailyLedgerProps) {
  ...
  const draw = (e: React.MouseEvent | React.TouchEvent) => {
      if (!isDrawing) return;
      e.preventDefault(); // Prevent scrolling while drawing on tablet
      const { x, y } = getCoordinates(e);
      const ctx = canvasRef.current?.getContext('2d');
      if (ctx) {
          ctx.lineTo(x, y);
  ...
  return (
    <section className="flex flex-col gap-6 w-full fade-in relative">
        <h2 className="font-headline text-headline-md text-on-surface font-bold">Logging Dashboard</h2>
        
        {/* Glassmorphism Container */}
        <div className="glass-panel ghost-border rounded-3xl p-6 flex flex-col gap-6">
           <div className="flex flex-col gap-2">
               ...
               <div className="flex gap-2">
                   ...
                   <button onClick={handleExtractAI} disabled={isExtracting || !autoFillInput.trim()} className="px-4 bg-primary text-on-primary-fixed rounded-xl text-xs font-bold transition-all hover:bg-primary-container active:scale-95">{isExtracting ? 'Parsing...' : 'Process'}</button>
               </div>
           </div>
               ...
               <div className="flex flex-col gap-2">
                   ...
                   <div className="grid grid-cols-3 bg-surface-container-lowest p-1 rounded-xl border border-white/5">
                       {(['Study', 'Revise', 'Exercise'] as SessionMode[]).map(m => (
                           <button key={m} onClick={() => props.logType !== m && props.setLogType(m)} className={`py-2 text-xs font-bold rounded-lg transition-all ${props.logType === m ? 'bg-primary text-on-primary-fixed shadow-md' : 'text-on-surface-variant hover:text-on-surface'}`}>{m}</button>
                       ))}
                   </div>
               </div>
           </div>
               ...
               <div className="flex flex-col gap-2 fade-in">
                   ...
                   <div className="grid grid-cols-3 bg-surface-container-lowest p-1 rounded-xl border border-white/5">
                       {['Quick Recap', 'Standard Review', 'Deep Dive'].map(depth => (
                           <button key={depth} onClick={() => setRevisionDepth(depth)} className={`py-2 text-xs rounded-lg transition-all ${revisionDepth === depth ? 'bg-tertiary/20 text-tertiary font-bold' : 'text-on-surface-variant hover:text-on-surface'}`}>{depth}</button>
                       ))}
                   </div>
               </div>
           )}
           ...
           {/* Redesigned Amber Friction Box with Scratchpad Trigger */}
           <div className="flex flex-col gap-2 p-4 rounded-xl border-2 border-warning/30 bg-warning/5 relative">
               <div className="flex items-center justify-between">
                   <label className="text-xs text-warning font-bold tracking-wider uppercase flex items-center gap-2">
                       <span className="material-symbols-outlined text-[16px]">gavel</span>
                       Friction Point Analysis
                   </label>
                   <button onClick={() => setShowScratchpad(true)} className="flex items-center gap-1 text-[10px] bg-warning/20 text-warning px-2 py-1 rounded-md hover:bg-warning/30 transition-colors font-bold cursor-pointer">
                       <span className="material-symbols-outlined text-[14px]">draw</span>
                       Digital Scratchpad
                   </button>
               </div>
               <textarea rows={2} value={frictionText} onChange={e => setFrictionText(e.target.value)} placeholder="Pinpoint equation breakdowns or concepts that cost time (Min 10 characters)..." className="w-full bg-surface-container-lowest border border-warning/20 rounded-xl p-3 text-sm outline-none text-on-surface focus:border-warning/60 transition-colors" />
           </div>

           <div className="flex flex-col gap-2">
               <label className="text-xs text-on-surface-variant font-semibold">Notes</label>
               <textarea rows={2} value={props.logNotes} onChange={e => props.setLogNotes(e.target.value)} placeholder="General log annotations..." className="w-full bg-surface-container-lowest border border-white/5 rounded-xl p-3 text-sm text-on-surface outline-none focus:border-primary/50" />
           </div>

           <button onClick={handleSaveLog} className="w-full py-4 rounded-xl bg-gradient-to-r from-primary to-primary-container text-on-primary-fixed font-bold text-sm tracking-wide shadow-lg transition-all hover:shadow-primary/30 active:scale-[0.99] cursor-pointer">
               Commit Logs to Database Pipeline
           </button>
        </div>
        ...
        {showScratchpad && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm fade-in">
                <div className="glass-panel w-full max-w-2xl h-[60vh] rounded-3xl flex flex-col overflow-hidden border border-white/10 shadow-2xl relative">
                    <div className="flex justify-between items-center p-4 border-b border-white/10 bg-surface-container-lowest">
                        ...
                        <div className="flex gap-2">
                            <button onClick={clearCanvas} className="text-xs bg-surface-container px-3 py-1 rounded-md text-on-surface-variant hover:text-on-surface transition-colors">Clear</button>
                            <button onClick={() => setShowScratchpad(false)} className="text-on-surface-variant hover:text-white"><span className="material-symbols-outlined">close</span></button>
                        </div>
                    </div>
                    {/* The drawing canvas designed for tablet styluses */}
                    <canvas 
                        ref={canvasRef}
