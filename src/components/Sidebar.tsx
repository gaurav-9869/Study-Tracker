...
export default function Sidebar({ currentTab, setTab, isOpen, setIsOpen, profileImg, userSettings }: SidebarProps) {
  ...
  // Clean, consolidated navigation matrix - extra standalone planner view removed completely!
  const navItems = [
  ...
  return (
    <>
      ...
      <aside className={`fixed top-0 bottom-0 left-0 w-64 z-[70] transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] border-r border-white/10 bg-black/40 flex flex-col justify-between p-4 md:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}
        style={{
      ...
      >
        <div className="flex flex-col gap-6 w-full">
          ...
          <div className="flex items-center gap-3 px-2 py-3 border-b border-white/10">
            <svg className="w-8 h-8 text-[var(--theme-primary,#10B981)] transition-colors duration-500" viewBox="0 0 240 240" fill="none" xmlns="http://www.w3.org/2000/svg">
