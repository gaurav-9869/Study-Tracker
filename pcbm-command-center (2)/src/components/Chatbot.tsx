...
If no action, send {"actions": [], "reply": "response"}. User input: "${input}"`;
        ...
        setChatMessages(prev => [...prev, {role: 'assistant', text: `Error: ${err.message}. Ensure GEMINI_API_KEY is hooked up in Secrets.`}]);
    }
  };
    ...
    <div className="fixed bottom-28 md:bottom-8 right-6 z-[60] flex flex-col items-end gap-4">
      <div className={`w-[350px] max-w-[90vw] h-[500px] glass-panel bg-surface-container-high border border-white/10 shadow-2xl flex flex-col overflow-hidden mb-4 animate-in slide-in-from-bottom-4 duration-300 ${chatOpen ? '' : 'hidden'}`}>
        ...
        <div className="p-4 bg-surface-container-lowest/50 border-t border-white/10">
          <form onSubmit={e => { e.preventDefault(); handleChatSuggest(); }} className="relative flex items-center">
            <input 
              type="text" 
              value={chatInput}
            ...
            <button type="submit" className="absolute right-2 w-8 h-8 flex items-center justify-center bg-primary text-on-primary-fixed rounded-full hover:shadow-[0_0_15px_rgba(0,191,255,0.4)] transition-all cursor-pointer">
